/*=================================================================
*
* CarveAllMod2.c - Simple threaded space carve
*                - carve one mask, then put the threads to work
*  
* Usage: ./CarveAllMod2 inputFile outputFile [<n> [trials]]
*
...
void carve(uint64_t i)
{
    ...
    for (iz = 0; iz<sZ; iz++){
		...
		if (V[iy + sYix + sYsXiz] == 1)
		{
                    ...
                    double qx = p0 * x + p3 * y + p6 * z + p9;
                    double qy = p1 * x + p4 * y + p7 * z + p10;
                    double qz = p2 * x + p5 * y + p8 * z + p11;
                    if (fabs(qz)<EPS){
                        continue;
                    }
